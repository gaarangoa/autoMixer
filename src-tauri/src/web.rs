...
async fn create_session() -> WebResult<MixProject> {
    Err(error(
        "Open an album folder in the AutoMixer desktop app before creating a song.".to_string(),
    ))
}
...
async fn import_audio_paths(
    ...
    for path in body.paths {
        latest = Some(
            store
                .add_source_file(&session_id, Path::new(&path))
                .map_err(error)?,
        );
    }
    latest.map(Json).map_or_else(
        || store.get_project(&session_id).map(Json).map_err(error),
        Ok,
    )
}
...
async fn assistant_request(
    ...
    let (response, project) =
        assistant::handle_assistant(state.config.clone(), project, request, observer)
            .await
            .map_err(error)?;
    {
        let store = state.store.lock().map_err(|item| error(item.to_string()))?;
