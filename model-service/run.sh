...
case "$AUTOMIXER_MODEL_RUNTIME" in
  llama_cpp)
    if [[ ! -x "$AUTOMIXER_MODEL_LLAMA_SERVER_BIN" ]]; then
      ...
      echo "Install it with Homebrew or override AUTOMIXER_MODEL_LLAMA_SERVER_BIN in config.env." >&2
      exit 1
    fi
    if [[ ! -f "$AUTOMIXER_MODEL_LLAMA_FILE" ]]; then
      echo "GGUF model not found: $AUTOMIXER_MODEL_LLAMA_FILE" >&2
      echo "Keep the weights outside Git and set AUTOMIXER_MODEL_LLAMA_FILE in config.env." >&2
      exit 1
    fi
  ...
  vllm)
    if [[ "$AUTOMIXER_MODEL_VLLM_BIN" == */* ]]; then
      VLLM_BIN="$AUTOMIXER_MODEL_VLLM_BIN"
    else
      VLLM_BIN="$(command -v "$AUTOMIXER_MODEL_VLLM_BIN" 2>/dev/null || true)"
    fi
    if [[ -z "$VLLM_BIN" || ! -x "$VLLM_BIN" ]]; then
      echo "vLLM executable not found: $AUTOMIXER_MODEL_VLLM_BIN" >&2
      echo "Set an absolute AUTOMIXER_MODEL_VLLM_BIN path in config.env for launchd." >&2
      exit 1
    fi

    IFS=',' read -r -a VLLM_ALIASES <<< "$AUTOMIXER_MODEL_ALIASES"
    VLLM_ARGS=(
      serve "$AUTOMIXER_MODEL_VLLM_ID"
      --host "$AUTOMIXER_MODEL_HOST"
      --port "$AUTOMIXER_MODEL_PORT"
      --max-model-len "$AUTOMIXER_MODEL_CONTEXT_SIZE"
      --gpu-memory-utilization "$AUTOMIXER_MODEL_VLLM_GPU_MEMORY_UTILIZATION"
      --served-model-name "${VLLM_ALIASES[@]}"
    )
    VLLM_ARGS+=(
      "${AUTOMIXER_MODEL_VLLM_EXTRA_ARGS[@]+"${AUTOMIXER_MODEL_VLLM_EXTRA_ARGS[@]}"}"
    )

    if [[ "$DRY_RUN" == "1" ]]; then
      automixer_model_print_command "$VLLM_BIN" "${VLLM_ARGS[@]}"
      exit 0
    fi
    automixer_model_ensure_state_dir
